...
const Submissions = () => {
  ...
  const fetchSubmissions = async (projectId = "", studentId = "", projectsList = projects) => {
    try {
  ...
  useEffect(() => {
    console.log('Current submissions state:', {
      flatSubmissions,
      studentSubmissions: flatSubmissions.filter(s => s.studentId === selectedStudentId)
    });
  ...
  const handleDeleteWeek = async (studentId, submissionId) => {
    ...
    try {
      ...
      const submissionToDelete = flatSubmissions.find(s => 
    ...
    <Box sx={{ backgroundColor: "#f9f9f9", minHeight: "100vh" }}>
      <Navbar2 />
        ...
        {loading ? (
          ...
          <TableContainer component={Paper} sx={{ mt: 4, mx: 4, borderRadius: 2 }}>
            <Table>
              <TableHead sx={{ backgroundColor: "grey" }}>
                <TableRow>
                  <TableCell sx={{ color: "#fff", fontWeight: "bold" }}>Student</TableCell>
                  <TableCell sx={{ color: "#fff", fontWeight: "bold" }}>Week</TableCell>
  ...
  {flatSubmissions.map((s, index) => {
    ...
    return (
      <TableRow key={index} sx={rowStyle}>
        <TableCell sx={{ fontWeight: "bold", color: "grey" , backgroundColor:'whitesmoke'}}>
          {s.studentName}
        </TableCell>
        ...
        <TableCell>
          <Stack spacing={1} direction="column">
            ...
            <Button
              ...
              variant="contained"
              color="error"
              onClick={() => handleDeleteWeek(s.studentId, s._id,s.projectId)}
            >
              Delete
            </Button>
          </Stack>
